...
//	timer1 fast pwm mode to output audio signal on OC1A/B pins (D9/10 for nano)
//	timer0 used to call handleButtons() function on comp B 1khz
//
...
void RF24Audio::begin(){
  ...
  RX();								   // Start listening for transmissions
  	...
  	TIMSK0 |= _BV(OCIE0B);			//t0 output compare B int enable
  #endif
}
...
void RF24Audio::timerStart(){
  ICR1 = 10 * (1600000/SAMPLE_RATE);                  //Timer will count up to this value from 0; timer TOP value
  TCCR1A = _BV(COM1A1) | _BV(COM1B0) | _BV(COM1B1);  //Enable the timer port/pin as output, A set on 0 reset at compare(noninvert), B set at compare reset at 0(invert) 
...
//	   	button handle
//******************************************
void handleButtons(){				//this function called only in TIMER0_COMPB_vect interrupt
...
//			TRANSMISSION BUTTON
	...
	#if !defined (RX_ONLY)
    		if(!transmitting){//do i need that when i check pin state?                         //Only do this if not already transmitting
      			transmitting = 1;                        //Set the transmitting variable
      			TX();                                    //Switch to TX mode
    		}
  	}else{                                       //if not transmitting then set into receive mod
	#endif
  		...
  		if(!streaming){                  			// If not actively reading a stream, read commands instead
  			if(radi.available() ){				// If a payload is available
			    ...
			    switch(buffer[0][0]){           	// Additional commands can be added here for controlling other things via radio command
 			    #if !defined (RX_ONLY)
			      case 'r': 			//Switch to TX mode if we received the remote tx command and this is radio 0 or 1
				      if(buffer[0][1] == 'R' && radioIdentifier < 2){ file:///home/aras/Downloads/walkie_talkie_Instructables/RF24Audio/RF24Audio.cpp
                TX();
            	}
            	break;
  			  #endif
...
ISR(TIMER1_COMPA_vect){                                    	// This interrupt vector sends the samples when a buffer is filled	
	if(buffEmpty[!whichBuff] == 0){                      		// If a buffer is ready to be sent
 		...
 		TIMSK1 &= ~(_BV(OCIE1A));            		// Disable this interrupt vector output compare A
...
// read ADC and reset in case buffer count
ISR(TIMER1_COMPB_vect){                                         // This interrupt vector captures the ADC values and stores them in a buffer
      ...
      buffer[whichBuff][buffCount] = bytH = ADCH;       // Read the high byte of the ADC register into the buffer for 8-bit samples
		...
		if(volMod < 0 ){
			OCR1A = bytH >> (volMod*-1);       	// Output to speaker at a set volume if defined
		}else{  	  
			OCR1A = bytH << volMod;
		}
      #endif
...
// 10 bit audio data
      ...
      bytH = ADCH;
      bitWrite(buffer[whichBuff][bytePos],bitPos, bitRead(bytH,0));  	//low bytes are stored in the first 25 bytes of the payload. additional 2 bits are stored in
      bitWrite(buffer[whichBuff][bytePos],bitPos+1, bitRead(bytH,1));	//pairs in the remaining bytes #25 to 31. Read the first and 2nd bits of the high register 
      bitPos+=2;						//into the payload.
...
void TX(){
	...
	TIMSK1 &= ~(_BV(ICIE1) | _BV(TOIE1));	// Disable capture & compare B int (the receive interrupts)
	#if defined (ENABLE_LED)
	...
	//arduino nano pin 14 -> A0 - microphone pin
	#else
      ...
      #if !defined (tenBit)
	ADMUX |= _BV(ADLAR);            			//Left-shift result so only high byte needs to be read, 8 bit data
      #else
