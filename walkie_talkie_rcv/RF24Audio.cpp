...
void RF24Audio::broadcast(byte radioID){
	...
	if(radioID == broadcastVal){
			radio.openWritingPipe(pipes[0]);						// Use the public multicast pipe
	}else{
  ...
  ISR(TIMER1_COMPB_vect){                                            // This interrupt vector captures the ADC values and stores them in a buffer
      ...
      buffer[whichBuff][buffCount] = bytH = ;                    // Read the high byte of the ADC register into the buffer for 8-bit samples
